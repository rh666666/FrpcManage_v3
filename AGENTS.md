...
**Tokens.** All color/space/radius come from `src/styles/_variables.scss`. Theme palettes live in `[data-theme="dark|light|aishenleishen"]` blocks; add themes only there plus `ThemeId` in Rust/TS — components use `var(--*)` only, no hardcoded palettes (especially log/terminal blues). `--accent` is primary buttons and `:focus-visible` only. `--ok/--warn/--err` are StatusDot, validation, and danger actions only. Switch on-state uses `--accent`, not green.
